...
public class GeneralSingleCookieService {
     ...
     */
    public Long retrieveWorkerByStudy(Cookie generalSingleCookie, Study study) {
     ...
     * future.
     */
    public Cookie create(Http.Request request, Study study, Worker worker) {
        Cookie oldCookie = request.cookie(COOKIE_NAME);
        String newCookieValue = addStudy(study, worker, oldCookie);
        return Cookie.builder(COOKIE_NAME, newCookieValue).withMaxAge(Duration.of(10000, ChronoUnit.DAYS))
