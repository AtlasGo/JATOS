...
/**
 * Implementation of JATOS' public API for studies that are started via MTurk. A MTurk run is done by a MTWorker or a
 * MTSandboxWorker.
 *
...
public class MTPublix extends Publix<MTWorker> implements IPublix {
    ...
    /**
     * Hint: Don't confuse MTurk's workerId with JATOS' workerId. They aren't the same. MTurk's workerId is generated by
     * MTurk and stored within the MTWorker. MTurk's workerId is used to identify a worker in MTurk. JATOS' workerId is
     * used to identify a worker in JATOS.
     */
    ...
    public Result startStudy(Http.Request request, Long studyId, Long batchId) throws PublixException {
        ...
        idCookieService.writeIdCookie(request, worker.get(), batch, studyResult);
     ...
     * quera has the key turkSubmitTo and it's value contains 'sandbox' it returns the MTSandboxWorker one - and
