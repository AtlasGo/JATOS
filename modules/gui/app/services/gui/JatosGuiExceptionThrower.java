...
/**
 * Class with convenience methods to throw a {@link JatosGuiException}. It checks whether the call is an Ajax one. It
 * puts the error message into the RequestScopeMessaging.
 *
...
public class JatosGuiExceptionThrower {
    ...
    @Inject
    JatosGuiExceptionThrower(Provider<Studies> studiesProvider, Provider<Home> homeProvider) {
     ...
     */
    public void throwAjax(String errorMsg, int httpStatus) throws JatosGuiException {
        Result result = Results.status(httpStatus, errorMsg);
        throw new JatosGuiException(result, errorMsg);
    }

    /**
     * Throws a JatosGuiException for an Ajax request (doesn't return a view but a simple text) with the exception's
     * message. The exception's type determines the response's HTTP status code.
     */
    ...
    /**
     * Throws a JatosGuiException for an Ajax request (doesn't return a view but a simple text) with the exception's
     * message. The HTTP's status code is taken from the parameter.
     */
    ...
    /**
     * Throws a JatosGuiException that either redirects to the given call if it's a non-Ajax request - or returns the
     * exception's message if it's a Ajax request. The exception's type determines the response's HTTP status code.
     */
    public void throwRedirect(Http.Request request, Exception e, Call call, boolean ajax) throws JatosGuiException {
        Result result;
        if (ajax) {
            int statusCode = getHttpStatusFromException(e);
            result = Results.status(statusCode, e.getMessage());
        } else {
            result = Results.redirect(call).flashing(Messages.ERROR, e.getMessage());
        }
        throw new JatosGuiException(result, e.getMessage());
    }

    /**
     * Throws a JatosGuiException. If it's a non-Ajax request, it puts the exception's message into the request scope
     * and returns the home view. If it's a Ajax request, it just returns the exception's message. The HTTP status code
    ...
    /**
     * Throws a JatosGuiException with the given error msg and HTTP status. If non Ajax it shows study's study view.
    ...
    /**
     * Throws a JatosGuiException. If it's a non-Ajax request, it puts the exception's message into the request scope
     * and returns the study's study view. If it's a Ajax request, it just returns the exception's message. The HTTP
